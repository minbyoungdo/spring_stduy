	...
	function getProductList(category) {
		$.get("/user/async/product/list/" + category, function(result) {
			...
			for (let i = 0; i < result.length; i++) {
				...
				buffer += '<a href="/user/User_One_Drink?category=' + result[i].pcategory + '"><img src="../resources/img/<c:out value="${product.imagefile}"/>" style="width:200px; height:250px" align="center"></a>';
				buffer += '<div>';
			...
			<c:forEach items="${list}" var="product">
				<div class="col-sm-4" style="width:250px;height:350px;border:1px solid;margin-left:10px;margin-right:10px;" >
					<div class ="col" style="margin-top:10px;">
						...
						<div>
							<p style="line-height:1"><c:out value="${product.pname}" /><p>
							<p style="line-height:1">rrrrrrrrr설명 : <c:out value="${product.description}"/><p>
						</div>
					</div>
				</div>
			</c:forEach>
